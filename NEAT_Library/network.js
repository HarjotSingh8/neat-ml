class Network {
  constructor(parameters) {
    ...
    this.completed = false; //may use to determine if the task required was completed (in some scenarios)
    this.fitness = 0;
    this.nodes = [];
    this.connections = [];
    this.gates = [];
  ...
  /**
   * disconnect from node from to node
   */
  disconnect(from, to) {
  ...
  /*evolve(set, options) {
    ...
    while (
      ...
      let fittest = neat.evolve(); //await to be used here
   ...
   */
  crossOver(network1, network2, equal) {
    if (
...
function NetworkcrossOver(network1, network2, equal) {
  if (
    network1.input !== network2.input ||
    network1.output !== network2.output
  ) {
