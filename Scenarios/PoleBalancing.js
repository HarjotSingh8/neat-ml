...
class PoleBalancing {
    ...
    constructor() {
        this.statesCount = this.statesCount || 100;
        this.states = [];
    ...
    initPopulation() {
        ...
        for(var i=0; i<this.statesCount; i++) {
            var startingCartPosition = random(0.45,0.55)
            var startingCartVelocity = random(-0.05,0.05)
            var startingPoleAngle = random(-0.05, 0.05)
            var startingForce = 0
            var startingColor = [random(0,255),random(0,255),random(0,255)];
            var cartPosition = []
            var cartVelocity = []
            var poleAngle = []
            var force = []
            var color = []
            for(var k=0; j<this.populationCount; j++) {

            }
            this.states.push({
                cartPosition:random(0.45,0.55),
                cartVelocity:random(-0.05,0.05),
    ...
    step(self, action) {
        ...
        var temp = (force + self.polemass_length*theta_dot**2*sintheta)/self.total_mass
        var thetaacc = (self.gravity*sintheta - costheta*temp)/(self.length*(4.0/3.0-self.masspole*costheta**2/self.total_mass))
    ...
    render() {
        ...
        var world_width = self.x_threshold * 2;
        var scale = screen_width/world_width;
        var carty = 100
        var polewidth = 10
        var polelen = scale * ( 2 * self.length );
        var cartwidth = 50;
        var cartheight = 30;
        ...
        background(0);
        fill(0);
        rect( (self.state[0]-self.cartwidth)*scale + screem_width/2, carty, self.cartwidth, self.cartheight);
        //pole
        stroke(255,0,0);
        strokeWeight(2);
        line( 
            self.state[0]*scale +  screem_width/2, 
            carty, 
            self.state[0]*scale + screem_width/2 + polelen*Math.sin(self.state[2]), 
            carty + polelen*Math.cos(self.state[2])
            )
    }
}