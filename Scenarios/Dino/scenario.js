class Scenario {
   ...
   */
  runIteration(individual) {}
  step() {
    for (var i = this.active.length - 1; i >= 0; i--) {
      runIteration(this.population[this.active[i]]);
      if (this.population[this.active[i]]) {
  ...
  drawEveryGeneration() {
    ...
    if (this.trained)
      for (var i = 0; i < 100; i += resolution) {
        for (var j = 0; j < 100; j += resolution) {
          fill(neat.population[0].activate([i / 100, j / 100])[0] * 255);
    ...
    for (
      ...
      var y = Math.abs(this.trainingData[i][0] - this.trainingData[i][1]);
      x = Math.abs(x - y);
      if (x < 0.01) {
        fill(0, 255, 0);
        rect(
          Math.floor(this.trainingData[i][0] * 100),
          Math.floor(this.trainingData[i][1] * 100),
          1,
          1
        );
      } else {
