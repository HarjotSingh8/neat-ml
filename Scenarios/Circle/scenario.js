class Scenario {
  ...
  drawEveryGeneration() {
    ...
    for (
      ...
      var y = Math.sqrt(
        Math.pow(0.5 - this.trainingData[i][0], 2) +
          Math.pow(0.5 - this.trainingData[i][1], 2)
      );
      //Math.abs(this.trainingData[i][0] - this.trainingData[i][1]);
      x = Math.abs(x - y);
      if (x < 0.01) {
        fill(0, 255, 0);
        rect(
...
function scenarioInit() {
  ...
  scenarioClass = new Scenario({
    populationSize: pop,
    trainingIterationsPerGeneration: 50,
    /*trainingData: [
      [0, 0],
      [0, 1],
      [1, 0],
