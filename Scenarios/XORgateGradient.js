  ...
  randomizePopulation() {
    for (var i = 0; i < this.populationCount; i++) {
      if(random(0,1)<0.5) {
        ...
        this.population[i].inputs = [random(0, 0.4), random(0.6, 1)];
      }
      else {
        if(random(0,1)<0.5)
        this.population[i].inputs = [random(0.6, 1), random(0, 0.4)];
        else
        this.population[i].inputs = [random(0.6, 1), random(0.6, 1)];
      }
...
function XORgateGradientInit() {
  ...
  scenarioReset = ()=>{scenarioClass.resetPopulation()};
  scenarioScore = scenarioClass.score;
  scenarioDraw = ()=>{XORgateGradientDraw()};
  
  console.log("success");
}

function XORgateGradientEvaluate() {
